function [raw,start_line] = eeg_load_mat_xdf_tori(filename,study_path,subject_log_name,subejct_listings,start_line,varargin)
...
matFile = load_xdf([filename]);

% first load the .mat file
%matFile = load(filename);
% streams = struct2cell(matFile);
% streams = streams{:};

%% Replace load xdf wiht load mat

% streams = load(filename);
% streams = streams.LSLout;

streams = matFile; clear matFile 
...
for s = 1:length(streams)
    ...
    if (strcmp(streams{s}.info.type,'Markers') || strcmp(streams{s}.info.type,'Events')) && ~ismember(streams{s}.info.name,args.exclude_markerstreams) 
        try
            ...
            % Load in all the text into Event
            for e=1:length(streams{s}.time_stamps)
                if iscell(streams{s}.time_series)
                    s_events(e).type = streams{s}.time_series{e}; % this is actually text not time stamps
                else
                    s_events(e).type = num2str(streams{s}.time_series(e));
                end
            end
                        
            fileID1 = fopen([study_path,'/','Presentation','/',subejct_listings,'/',subject_log_name{1}],'r');
            logMatrix = textscan(fileID1,'%s','delimiter','\n');
            logMatrix = logMatrix{1,1};
            fclose(fileID1);
            numRowsLog_orig = size(logMatrix,1);
            
            %if k == 1
                start_pts = start_line;
                numRowsLog = numRowsLog_orig;
            %elseif k == 2
            %    start_pts = length(s_events)+1;
            %    numRowsLog = length(s_events);
            %end
                
            cnt = 1;
            for k = start_pts:numRowsLog
                [~,remain_2] = strtok(logMatrix{k});
                [~,rem2] = strtok(remain_2);
                [rem1,~] = strtok(rem2); % Index event string                  
                s_events(cnt).type = rem1; % Replace indexed event string with meaningful name
                cnt = cnt+1;
            end
            ...
            for e=1:length(streams{s}.time_stamps)
                s_events(e).type = s_events(e).type;  %eventTypesOut{e};
                [~, s_events(e).latency] = min(abs((stream.time_stamps - streams{s}.time_stamps(e))*2048));
            end
